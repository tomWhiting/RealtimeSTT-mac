...
def test_stella_custom_model():
    ...
    print("📊 Test Results:")
    print(
        f"   Custom Stella model: {'✅ SUCCESS' if wake_word_detected else '❌ NOT TESTED'}"
    )
    print(
        f"   Wake word detection: {'✅ SUCCESS' if wake_word_detected else '❌ NOT TESTED'}"
    )
    print(
        f"   Transcription: {'✅ SUCCESS' if transcription_received else '❌ NOT TESTED'}"
    )
