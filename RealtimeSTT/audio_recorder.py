        ...
        if wake_words or wakeword_backend in {
            ...
            if wake_words and self.wakeword_backend in {"pvp", "pvporcupine"}:
                try:
                    ...
                    if not access_key:
                        raise ValueError(
                            "PORCUPINE_ACCESS_KEY environment variable is required for Porcupine 3.0+"
                        )
                    ...
                    if custom_model_paths and not builtin_keywords:
                        ...
                        self.porcupine = pvporcupine.create(
                            ...
                            sensitivities=self.wake_words_sensitivities[
                                : len(builtin_keywords)
                            ],
                        )
                    else:
                        # Mixed - not supported, use built-in only
                        logger.warning(
                            "Mixed custom and built-in wake words not supported. Using built-in keywords only."
                        )
                        self.porcupine = pvporcupine.create(
                            access_key=access_key,
                            keywords=builtin_keywords,
